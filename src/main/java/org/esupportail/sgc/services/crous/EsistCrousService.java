...
public class EsistCrousService {
	...
	public List<Long> compute(User user) {
		if(crousService.isEnabled() && crousRuleDaoService.countCrousRules()==0) {
		...
		for(CrousRule rule : crousRuleDaoService.findAllCrousRules("priority", "desc")) {
	...
	public void updateCrousRules() {
		for(CrousRuleConfig crousRuleConfig : crousRuleConfigDaoService.findAllCrousRuleConfigs()) {
			...
			for(CrousRule crousRule : crousRuleDaoService.findAllCrousRules(crousRuleConfig)) {
	...
	public void deleteCrousRuleConfig(Long id) {
		CrousRuleConfig crousRuleConfig = crousRuleConfigDaoService.findCrousRuleConfig(id);
		for(CrousRule crousRule : crousRuleDaoService.findAllCrousRules(crousRuleConfig)) {
            crousRuleDaoService.remove(crousRule);
		}
        crousRuleConfigDaoService.remove(crousRuleConfig);
	}
}
