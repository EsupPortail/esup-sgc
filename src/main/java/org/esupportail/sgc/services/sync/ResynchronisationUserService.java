...
public class ResynchronisationUserService {
	...
	public boolean synchronizeUserInfoNoTx(String eppn) {
		...
		if(UserInfoService.SynchroCmd.NONE.equals(syncUserInfoServiceFlag)) {
		...
		} else if(dummyUser.getDueDate()!=null && user.getDueDate()!=null && dummyUser.getDueDate().isBefore(LocalDateTime.now()) && user.getDueDate().isBefore(LocalDateTime.now())) {
			...
			if (dummyUser.getExternalCard().getCsn() == null || dummyUser.getExternalCard().getCsn().isEmpty()) {
				for (Card card : user.getCards()) {
					if (card.getExternal() && card.isEnabled() && dummyUser.getDueDate().isAfter(LocalDateTime.now())) {
				...
				if (externalCard != null) {
					if (dummyUser.getExternalCard().getCsn() == null || dummyUser.getExternalCard().getCsn().isEmpty()) {
						...
						if(photo.getBigFile().getBinaryFile() != null) {
                            ...
                            bigFileDaoService.setBinaryFile(externalCard.getPhotoFile().getBigFile(), importExportCardService.loadNoImgPhoto());
						...
						if (Etat.DISABLED.equals(externalCard.getEtat()) || Etat.CADUC.equals(externalCard.getEtat()) && (dummyUser.getDueDate() == null || dummyUser.getDueDate().isAfter(LocalDateTime.now()))) {
							userInfoService.setAdditionalsInfo(user, null);
			...
			if(!fieldNotEquals.isEmpty() && (user.getDueDate() != null || dummyUser.getDueDate() != null)) {
				...
				if(dummyUser.getDueDate() != null && dummyUser.getDueDate().isBefore(LocalDateTime.now()) || dummyUser.getDueDate() == null && user.getDueDate() != null && user.getDueDate().isBefore(LocalDateTime.now())) {
		...
		for(Card card : user.getCards()) {
			...
			if(card.getDueDate()==null || card.getDueDate().isAfter(user.getDueDate()) || (card.isEnabled() || CardEtatService.etatsRequest.contains(card.getEtat())) && !user.getDueDate().equals(card.getDueDate())) {
				card.setDueDate(user.getDueDate());
                cardDaoService.merge(card);
				accessControlMustUpdate = true;
			}
			// if card was out_of_date we check due_date
			if(Etat.CADUC.equals(card.getEtat()) && card.getDueDate().isBefore(user.getDueDate())) {
				card.setDueDate(user.getDueDate());
                cardDaoService.merge(card);
			}
			// if card must be reenabled we enable it
			if(Etat.CADUC.equals(card.getEtat()) && card.getDueDate().isAfter(LocalDateTime.now())) {
				cardEtatService.setCardEtat(card, Etat.DISABLED, null, null, false, true);
			}
			// if card must be out_of_date we invalidate it
			if((Etat.ENABLED.equals(card.getEtat()) || Etat.DISABLED.equals(card.getEtat()) || Etat.ENCODED.equals(card.getEtat())) && user.getDueDate().isBefore(LocalDateTime.now())) {
