...
public class DbToolService {
	...
	private void upgradeIfNeeded(AppliVersion appliVersion) {
		...
		try{
            ...
            if("2.6.x".equals(esupSgcVersion)) {
				...
				String sqlUpdate = "update user_account set update_date='1970-01-01' where update_date is null;";
				doSqlUpdate(sqlUpdate);
				esupSgcVersion = "3.0.x";
            }
			if("3.0.x".equals(esupSgcVersion)) {
			...
			if("3.2.x".equals(esupSgcVersion)) {
				String sqlUpdate = "ALTER TABLE user_account ADD COLUMN IF NOT EXISTS first_request_free BOOLEAN DEFAULT TRUE;";
				sqlUpdate += "update user_account set first_request_free=true;";
	...
	void doSqlUpdate(String sqlUpdate) throws SQLException {
